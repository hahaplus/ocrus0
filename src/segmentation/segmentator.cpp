...
void Segmentator::getRow(cv::Mat &image, OcrDetailResult* seg_result) {
  ...
  for (auto ax : axises) {
    ...
    for (auto unit_index : row_members[row_index]) {
      if (unit_line_num[unit_index] == -1) {
        unit_line_num[unit_index] =
            cnt > 3 ? *other_row.begin() // merge row
                : row_index;
      }
    }
  }
...
void Segmentator::handleEachRowInit(Mat &image, map<int, vector<ResultUnit> > &row_map)
{
     confidence.clear();
     result_map.clear();
     ...
     for (auto &row : row_map)
     {
              ...
              for (int size = 1; size <= 3; size++)
              {
                  for (int i = 0; i < (int)rs.size() - size + 1; i++)
                  {
                       ...
                       for (int j = rs[i].bounding_box[0].x; j <= rs[i + size - 1].bounding_box[1].x; j++)
                       {
                           if (x_histogram[j] == 0)
                           {
                               white_cnt++;
...
void Segmentator::handleEachRow(cv::Mat &img, int row_index,vector<ResultUnit> &row_units, vector<vector<float> > &confi)
{
     ...
     for (int size = 1; size <= row_units.size(); size++)
     {
         for (int i = 0; i < (int)row_units.size() - size + 1; i++)
         {
            ...
            {
                if (confi[i][i + size - 1] < (confi[i][k] + confi[k+1][i + size - 1]) / 2.0)
                {
                    confi[i][i + size - 1] = (confi[i][k] + confi[k+1][i + size - 1]) / 2.0;
                    choice[i][i + size - 1] = k;
                }
            }
         }
...
void Segmentator::getAllTheSplitIndex(set<int>& split_index_set, int start, int end, vector<vector<int> > &choice)
{
